...
STEPS = list(range(0, 121, 6))               # 상층(925~200) 6h, +120h — 사용자: 현행 유지
SFC_END = {"ECMWF": 240, "GFS": 384}          # 지상장은 모델 한계까지: 120h 이후 12h(00/12UTC 유효) 2026-09-08

def sfc_steps(run_hour: int, model: str) -> list[int]:
    return STEPS + [s for s in range(121, SFC_END[model] + 1) if (run_hour + s) % 12 == 0]
...
def fetch_ecmwf(source: str, workers: int = 21, run: str | None = None) -> list[str]:
    ...
    for name, req in (("pl", req_low), ("pl2", req_high),
                      ("sfc", {"type": "fc", "stream": "oper", "levtype": "sfc",
                               "param": ["msl", "2t"], "step": sfc_steps(latest.hour, "ECMWF"),
...
def gfs_url(ymd: str, hh: str, step: int, sfc_only: bool = False) -> str:
    p = [f"dir=%2Fgfs.{ymd}%2F{hh}%2Fatmos", f"file=gfs.t{hh}z.pgrb2.0p25.f{step:03d}"]
    if sfc_only:                                   # 120h 이후: 지상장(해면기압·2m 기온)만
        p += ["var_TMP=on", "var_PRMSL=on"]
    else:
        p += ["var_HGT=on", "var_TMP=on", "var_PRMSL=on", "var_UGRD=on", "var_VGRD=on", "var_RH=on"]
...
def fetch_gfs(run: str | None = None) -> str:
    ...
    tmp = target + ".part"
    n_ok = 0
    steps = sfc_steps(int(hh), "GFS")
    with open(tmp, "wb") as dst:
        for step in steps:
            ok = False
            for attempt in range(3):
                try:
                    r = s.get(gfs_url(ymd, hh, step, sfc_only=step > 120), timeout=120)
