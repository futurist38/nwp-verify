...
def main():
    ...
    if len(key) != 22:      # 값은 찍지 않고 문자 종류만 — 시크릿에 무엇이 섞였는지 진단 (2026-09-16)
        kinds = "".join("a" if c.isalnum() else ("s" if c.isspace() else "?") for c in key)
        odd = sorted({f"U+{ord(c):04X}" for c in key if not c.isalnum()})
        print(f"[KIM-NC] 기관키 문자 종류: {kinds} (a=영숫자 s=공백 ?=기타 {odd})", flush=True)
